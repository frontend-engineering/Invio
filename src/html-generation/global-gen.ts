...
export class GlobalDataGenerator
{
	...
	public static getGlobalGraph(minRadius: number, maxRadius: number, root: string = ''): {nodeCount: number, linkCount: number, radii: number[], labels: string[], paths: string[], linkSources: number[], linkTargets: number[]}
	{
		...
		if (typeof root !== 'string') {
			throw new Error('Invalid root dir when generating graph');
		}
		...
		paths = indexedRadii.map(r => InvioSettingTab.settings.makeNamesWebStyle ? Path.toWebStyle(paths[r.index]) : paths[r.index]);
		paths = paths.map(p => 
			{
