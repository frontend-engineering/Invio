...
export const publishFile = async (
    ...
    path: string,
    basePath: Path,
    password: string = "", // Keep password parameter for futher reuse
    settings: any,
    ...
    // get files to export
    let allFiles = vault.getMarkdownFiles();
    // if we are at the root path export all files, otherwise only export files in the folder we are exporting
    allFiles = allFiles.filter((file: TFile) => new Path(file.path).directory.asString.startsWith(basePath.asString) && file.extension === "md");
    ...
    if (exportedFile) {
        const toUploads = exportedFile.downloads.map(d => {
        ...
        await Utils.downloadFiles(externalFiles, htmlFilePath.directory);
        ...
        try {
            ...
            await Promise.all(resPromise).then(result => {
                ...
                if (InvioSettingTab.settings.openAfterExport && (triggerSource === 'manual')) {
                    log.info("Opening: " + exportedFile.exportPathAbsolute.asString);
